      ...
      case ${fileextension} in
        war|ear)
          ...
          if [ $? -eq 0 ]; then
             ...
             #failure "When systems come to be far from points of equilibrium, they reach bifurcation points, wherein multiple, as opposed to unique, solutions, to instability become possible. " ${INFOABOUTEXIST} " : Resumindo esta biblioteca está dentro do artefacto."
             #failure "You can not have the sun in the threshing-floor and water on the turnip farm. " ${INFOABOUTEXIST} " : Resumindo esta biblioteca está dentro do artefacto."
...
#manjasboy######
#   UNZIPEAR=`/usr/bin/unzip -t *.ear 2>/dev/null` > ${TMPCHECKFILE}
#   if [ $? -eq 0 ]; then
#   else  
#   UNZIPEWAR=`/usr/bin/unzip -t *.war 2>/dev/null` > ${TMPCHECKFILE}
#   fi
...
#################
#DRIVERSINSIDE=`/bin/grep ojdbc ${TMPCHECKFILE}`
   ...
   #STATICINSIDE=`/bin/grep -i '.png   OK$\|.gif   OK$\|.img   OK$\|.svg   OK$\|.js   OK$\|.css   OK$\|.pdf   OK$\|.jpg   OK$\|.jpeg   OK$' ${TMPCHECKFILE}`
...
check_by_vulnerability_struts()
{
   ...
   if [ $RETURNCODE -eq 0 ]; then
      ...
      /usr/bin/unzip -d $TMPARTEFACTUNZIPED $ARTEFACTFILE > /dev/null 2>&1
...
app_deploy()
{
  ...
  if [[ ${COUNTERFILESREPEATED} -gt 1 ]]; then
     #failure "Used * in the process of deploy with two modules available. >> " ${CHECKFILEDUPLICATE[@]}
     failure "Two or more are good only in some situations, but not here. >> " ${CHECKFILEDUPLICATE[@]}
  fi

  #echo "##########################"
  #multifiles=`echo ${#FILELIST[@]}`
  #echo "ARRAY LENGTH >> " ${multifiles}
  #echo "ARRAY LIST >> " ${FILELIST[@]}
  #echo "##########################"
