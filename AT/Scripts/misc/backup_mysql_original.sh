...
DUMP_DIR="/var/backups/${DB_HOST:-localhost}/mysql"

# log files
LOG="/var/log/backups/${DB_HOST:-localhost}_mysql.log"
ERROR_LOG="/var/log/backups/${DB_HOST:-localhost}_mysql_error.log"
...
for db in $(${MYSQL} -h ${DB_HOST:-localhost} -u ${DB_USER} -p${DB_PASSWORD} -Ns -e 'show databases'); do
	...
	# prepare dump directory
  	...
  	if [ ${db} = mysql -o ${db} = information_schema -o ${db} = performance_schema ]; then
    		${MYSQLDUMP} --skip-lock-tables -h ${DB_HOST:-localhost} -u ${DB_USER} -p ${DB_PASSWORD} ${db} 2>> ${ERROR_LOG} > ${DUMP_DIR}/${db}/${db}-${ROTATION}.sql;
  	else
    		${MYSQLDUMP} -h ${DB_HOST:-localhost} -u ${DB_USER} -p${DB_PASSWORD} -FER ${db} 2>> ${ERROR_LOG} > ${DUMP_DIR}/${db}/${db}-${ROTATION}.sql;
  	fi
