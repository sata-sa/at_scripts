...
DOMAIN=$2
SERVER=$3
TRANSACTION="SERVER_DEL(${DOMAIN}:${SERVER})"
JOBID=${RANDOM}

#########
# Paths #
#########
SERVER_DEL_PYTHON_FILE=${HOME}/etc/py/server_del.py
...
{
  # Check Jython scrits
  if [ ! -e "${SERVER_DEL_PYTHON_FILE}" ]; then
    failure "Jython file ${SERVER_DEL_PYTHON_FILE} not found"
  fi
}
...
{
cat << EOF
USAGE: $0 <ENVIRONMENT> <DOMAIN NAME> <SERVER NAME>

  ENVIRONMENT         - The environment where the server exists. Available options are: PRD/QUA/DEV/SANDBOX
  DOMAIN NAME         - The domain where the server exists
  SERVER NAME         - The name of the server to delete
...
check_db_weblogic()
{
  ...
  if [ -z ${DBQUERY} ] && [ -z ${XMLQUERY} ]; then
    failure "Server ${SERVER} not present in ${ENV} database and WebLogic domain ${DOMAIN}"
  elif [ -z ${DBQUERY} ]; then
    failure "Server ${SERVER} not present in ${ENV} database"
  elif [ -z ${XMLQUERY} ]; then
    failure "Server ${SERVER} not present in WebLogic domain ${DOMAIN}"
  fi
}
...
delete_server()
{
  info "Attempting to delete server ${SERVER}"
  "$JAVA_HOME/bin/java" ${JVM_ARGS} weblogic.WLST ${SERVER_DEL_PYTHON_FILE} ${ADMIN_HOST} ${ADMIN_PORT} ${ADMIN_PASSWORD} ${SERVER}

  if [ $? -gt 0 ]; then
    failure "Unable to delete server ${SERVER}"
  else
    set-env-info.pl ${ENV} -d s,${SERVER}
    info "Server ${SERVER} deleted from WebLogic domain ${DOMAIN} and ${ENV} database"
  fi
}
