...
STARTCLUSTER_PYTHON_FILE="/home/weblogic/etc/py/start_cluster.py"
...
for APP in "${UDOMAIN[@]}"; do
   ...
   info "The "$CONNTARGET" will start..."

   if [ -z $DOMAIN ]; then
      failure "Application does not exist!!!"
   fi

"$JAVA_HOME/bin/java" ${JVM_ARGS} weblogic.WLST ${STARTCLUSTER_PYTHON_FILE} ${WLHOST} ${WLPORT} ${WLPASS} ${CONNTARGET}
