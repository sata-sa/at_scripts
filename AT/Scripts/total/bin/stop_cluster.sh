...
SHUTDOWNCLUSTER_PYTHON_FILE="/home/weblogic/etc/py/stop_cluster.py"
...
if [ -z ${WEBAPP} ]; then
   failure "You must provide your application/cluster name"
fi

domtarg()
{
if [[ ${WEBAPP} == *?[Cc]luster?? ]]; then
   for APP in "${WEBAPP[@]}"; do
      APNA=$APP
      DOMAIN=`echo $APP | sed -e 's/[Cc]luster.*$//g'`
      DOMAINCHECK=`get-env-info.pl prd | awk -F: '{print $2}' | grep ${DOMAIN}`
      ...
      info "The "$WEBAPP" will shutdown..."
      echo "$JAVA_HOME/bin/java" ${JVM_ARGS} weblogic.WLST ${SHUTDOWNCLUSTER_PYTHON_FILE} ${WLHOST} ${WLPORT} ${WLPASS} ${APP}
      "$JAVA_HOME/bin/java" ${JVM_ARGS} weblogic.WLST ${SHUTDOWNCLUSTER_PYTHON_FILE} ${WLHOST} ${WLPORT} ${WLPASS} ${APP}
   done
else
   for APP in "${WEBAPP[@]}"; do
      APNA=$APP
      ...
      echo "$JAVA_HOME/bin/java" ${JVM_ARGS} weblogic.WLST ${SHUTDOWNCLUSTER_PYTHON_FILE} ${WLHOST} ${WLPORT} ${WLPASS} ${CONNTARGET}
